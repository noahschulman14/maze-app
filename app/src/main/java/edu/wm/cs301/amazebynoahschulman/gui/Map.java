...
public class Map {
	 ...
	 * @param mazePanel graphics handler to manipulate screen
	...
	private void drawMap(MazePanel mazePanel, int px, int py, int walkStep,
		...
		for (int y = minY; y <= maxY; y++)
			for (int x = minX; x <= maxX; x++) {
				...
				if (x < mazeWidth)
					drawHorizontalLine(mazePanel, showMaze, x, y, startX, startY);
				if (y < mazeHeight)
					drawVerticalLine(mazePanel, showMaze, x, y, startX, startY);
			}
	 ...
	 */
	private void drawVerticalLine(MazePanel mazePanel, boolean showMaze, int x, int y, int startX, int startY) {
		if (hasAVerticalWall(x, y) && 
				(seenWalls.hasWall(x, y, CardinalDirection.West) || showMaze)) {
			// WALLS THAT HAVE BEEN SEEN BEFORE: LTGRAY, DEFAULT: DKGRAY
	 ...
	 * @param startY the y coordinate for drawing
	 */
	private void drawHorizontalLine(MazePanel mazePanel, boolean showMaze, int x, int y, int startX,
			int startY) {
